...
def get_active_courses(request):
    ...
    for activity in activities:
        if activity.status == activity.INPROGRESS and activity.course not in courses:
...
@api_view(['POST'])
def track_progress(request, course_slug, lesson_slug):
...
@api_view(['POST'])
def track_done(request, course_slug, lesson_slug):
    course = Course.objects.get(slug=course_slug)
    lesson = Lesson.objects.get(slug=lesson_slug)
